...
String Rational::toShortString() const{
    if(b==1)
        ...
        char buffer[64];
        int ratio;
        if(!a.part.size()) return "0";
        sprintf(buffer,"%1.*f",10,(double)a.part.back()/(double)b.part.back());
        switch(ratio = (a.part.size()-1)-(b.part.size()-1)){
            case 0:  return ((negative())?"-":"")+String(buffer);
            case 1:  return ((negative())?"-":"")+String(buffer)+"*(2^32)";
            default: return ((negative())?"-":"")+String(buffer)+"*(2^32^"+ToString(ratio)+")";
        }
    }
    else if(a.size()>b.size())
        return "inf";
