...
void HierarchyComponent::script_registration() {
  ...
  L_SCRIPT_METHOD(HierarchyComponent, "parent", 1, parent(c.param(0).get<Transform*>()));
  L_SCRIPT_METHOD(HierarchyComponent, "translation", 1, translation(c.param(0).get<Vector3f>()));
  L_SCRIPT_METHOD(HierarchyComponent, "rotation", 2, rotation(c.param(0).get<Vector3f>(), c.param(1).get<float>()));
  L_SCRIPT_METHOD(HierarchyComponent, "rotate", 2, rotate(c.param(0).get<Vector3f>(), c.param(1).get<float>()));
  L_SCRIPT_METHOD(HierarchyComponent, "rotate_absolute", 2, rotate_absolute(c.param(0).get<Vector3f>(), c.param(1).get<float>()));
}
