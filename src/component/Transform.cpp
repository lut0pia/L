...
void Transform::script_registration(){
  ...
  L_COMPONENT_RETURN_METHOD(Transform, "get_position", 0, position());
  ...
  L_COMPONENT_RETURN_METHOD(Transform, "to_absolute", 1, to_absolute(c.param(0)));
  L_COMPONENT_METHOD(Transform, "set_position", 1, position(c.param(0).get<Vector3f>()));
  L_COMPONENT_METHOD(Transform, "set_rotation", 2, rotation(c.param(0).get<Vector3f>(), c.param(1).get<float>()));
  L_COMPONENT_METHOD(Transform, "move", 1, move(c.param(0).get<Vector3f>()));
  L_COMPONENT_METHOD(Transform, "move_absolute", 1, move_absolute(c.param(0).get<Vector3f>()));
  L_COMPONENT_METHOD(Transform, "rotate", 2, rotate(c.param(0).get<Vector3f>(), c.param(1).get<float>()));
  L_COMPONENT_METHOD(Transform, "rotate_absolute", 2, rotate_absolute(c.param(0).get<Vector3f>(), c.param(1).get<float>()));
}
