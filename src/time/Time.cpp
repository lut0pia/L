...
Stream& L::operator<<(Stream &s, const Time& v) {
  ...
  if(hours) {s << numberToString<10>(hours,2) << 'h'; if(++c) return s;}
  long long mins((us/60000000LL)%60LL);
  if(mins) {s << numberToString<10>(mins,2) << 'm'; if(++c) return s;}
  long long secs((us/1000000LL)%60LL);
  if(secs) {s << numberToString<10>(secs,2) << 's'; if(++c) return s;}
  long long msecs((us/1000LL)%1000LL);
  if(msecs) {s << numberToString<10>(msecs,3) << "ms"; if(++c) return s;}
  long long usecs(us%1000LL);
  if(usecs) {s << numberToString<10>(usecs,3) << "us"; if(++c) return s;}
  return s;
}
