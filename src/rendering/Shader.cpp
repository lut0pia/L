...
void Shader::reflect(Intermediate& intermediate) {
  ...
  for_opcodes(binary, size, OpVariable, [&](const uint32_t* variable) {
    ...
    switch(variable[3]) {
      case 0: binding.type = UniformConstant; break;
      case 1: binding.type = Input; break;
      case 2: binding.type = Uniform; break;
      case 9: binding.type = PushConstant; break;
      default: return;
    }
    ...
    if(binding.type==Uniform) { // Create bindings for every member of block
      binding.type = None;
      for_opcodes(binary, size, OpMemberDecorate, [&](const uint32_t* decoration) {
  ...
  for(uintptr_t i(0); i<intermediate.bindings.size(); i++) {
    ...
    } else if(intermediate.bindings[i].type==Input) {
      // Compute Input offset by checking all Input with lesser index
      Binding& binding(intermediate.bindings[i]);
      for(const Binding& other_binding : intermediate.bindings) {
        if(other_binding.type==Input && other_binding.index<binding.index) {
